...
public:
    ...
    bool allocate(UMatData *data, int accessflags, UMatUsageFlags usageFlags) const {
        throw "NOT IMPLEMENTED";
    }

    UMatData *allocate(int dims, const int *sizes, int type, void *data, size_t *step, int flags, UMatUsageFlags usageFlags) const {
        throw "NOT IMPLEMENTED";
    }
...
cv::Mat NDArrayConverter::toMat(const PyObject *o)
{
    ...
    if( ndims > 2)
    {
    	...
    	cout << "toMat! FAIL 4.5" << endl;
    }

    m = Mat(ndims, size, type, PyArray_DATA(o), step);

    if( m.data ) {
        m.u = new cv::UMatData(&g_numpyAllocator);
        m.u->refcount = Py_REFCNT(o);
        m.addref(); // protect the original numpy array from deallocation
                    // (since Mat destructor will decrement the reference counter)
    };
    m.allocator = &g_numpyAllocator;

    cout << "toMat! STEP 9" << endl;
...
PyObject* NDArrayConverter::toNDArray(const cv::Mat& m)
{
    ...
    cout << "toNDArray! STEP 1 " << !p->u->refcount << endl;
    if(!p->u->refcount || p->allocator != &g_numpyAllocator)
    {
        temp.allocator = &g_numpyAllocator;
        m.copyTo(temp);
        p = &temp;
    cout << "toNDArray! STEP 2 "<< endl;
    }
    p->addref();
    cout << "toNDArray! STEP 3 " << !p->u->refcount << endl;
